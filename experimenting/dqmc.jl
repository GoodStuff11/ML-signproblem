...
function run_hubbard_square_simulation(sID, U, μ, β, L, N_burnin, N_updates, N_bins; filepath = ".")
    ...
    bond_px = latut.Bond(orbitals = (1,1), displacement = [1,0])
    ...
    bond_py = latut.Bond(orbitals = (1,1), displacement = [0,1])

    # Add the nearest-neighbor bond in the +y direction.
    bond_py_id = add_bond!(model_geometry, bond_py)

    # Define the nearest-neighbor bond in the -x direction.
    bond_nx = latut.Bond(orbitals = (1,1), displacement = [-1,0])

    # Add nearest-neighbor bond in the -x direction.
    bond_nx_id = add_bond!(model_geometry, bond_nx)

    # Define the nearest-neighbor bond in the -y direction.
    bond_ny = latut.Bond(orbitals = (1,1), displacement = [0,-1])
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # Run the simulation.
    run_hubbard_square_simulation(sID, U, μ, β, L, N_burnin, N_updates, N_bins; filepath=raw"D:\datasets\dqmc_hubbard")
