...
# Returns
	# (s_i, s_j,): N-bit ints for the length-nc creation and length-na annihilation strings; size: (N choose nc+na)*((nc+na) choose nc)
	# s_i = ∑_m 2^{i_m} ;  ∆ =  ∑ q_cre - ∑ q_ann
def singleSpinChannelCombos(N,nc,na,allow_overlap=False, returnMomTransfer=False,Lvec=None):
	# Input validation ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	too_many = (nc>N or na>N) if allow_overlap else (nc+na>N) # more spin-σ ops than sites --> term vanishes
	if too_many:
		cre = np.array([], dtype=dt_mb); ann = np.array([], dtype=dt_mb)
		return (cre, ann, np.array([], dtype=dt_site)) if returnMomTransfer else (cre, ann)

	# Get (i_σ,j_σ) combos; let D = # of (i_σ,j_σ) combos
	if allow_overlap: 	# get I,J combos if allow overlap ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# i_σ and j_σ chosen INDEPENDENTLY: any nc-subset × any na-subset may share sites -> full Cartesian product
		cre_int, cre_occ = getReducedHilSpace(N, nc, returnOcc=True)                                    # (Dc,), (Dc, nc); Dc = (N choose nc)
		ann_int, ann_occ = (cre_int, cre_occ) if na == nc else getReducedHilSpace(N, na, returnOcc=True) # (Da,), (Da, na); Da = (N choose na)
		cre, ann = cartesian_prod(cre_int.astype(dt_mb), ann_int.astype(dt_mb))                          # (D=Dc*Da,) each; cre index outer (changes slowest), ann idx inner (changes fastest)
		if not returnMomTransfer:
			return cre, ann

		# Δ = ∑q_cre − ∑q_ann: sum momenta within each subset ONCE, then broadcast-subtract over the product (shared sites cancel automatically)
		qsum_cre = np.stack(np.unravel_index(cre_occ, Lvec), axis=0).sum(2)                              # (d, Dc); ∑q over the nc cre sites; stack gives d x D x nc = qvecs for each (D x nc) (i_σ,j_σ')
		qsum_ann = qsum_cre if na == nc else np.stack(np.unravel_index(ann_occ, Lvec), axis=0).sum(2)    # (d, Da)
		delta = qsum_cre[:, :, None] - qsum_ann[:, None, :]                                              # (d, Dc, Da)
		delta_int = np.ravel_multi_index(delta, Lvec, mode='wrap').astype(dt_site).ravel()               # (D=Dc*Da,); ∆ folded to BZ, flattened
		return cre, ann, delta_int

	else:  # get i_σ, j_σ combos without overlap ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# Choose nc+na idcs for i_σ ∪ j_σ for spin-σ channel: every N-bit mask of size nc+na, with its sorted site list
		...
		# Compute momentum transfer ∆ = ∑ q_cre - ∑ q_ann, turned back into index in [0,N-1]
		qvecs = np.stack(np.unravel_index(supp_occ, Lvec), axis=-1)  # (D, nc+na, d); d-dim wavevector of each occupied site
		sign = 2*cre_bits - 1  # (K, nc+na); +1 on creator slots, −1 on annihilator slots (must stay signed)
		delta = np.einsum('dja,kj->adk', qvecs, sign)  # (d, D, K); 'a' = lattice axis
...
# Enumerates idcs for order-p fermionic gate c^†_{i_1,σ_1}...c^†_{i_p,σ_p} c_{j_1,σ'_1}...c_{j_p,σ'_p}
# Inputs
	# p: excitation order; # of c^†'s = # of c's ==> total number conserving
	# Lvec: lattice dimensions; e.g. (3,2) for 3x2 lattice
	# allow_overlap: bool; if false, then {iσ}∩{jσ'} = ∅
	# conserve_[prop]: list of which ops to conserve; {'sz','mom','su2'}
...
def enumerate_ferm_excitations(p, Lvec, allow_overlap = False, conserve_mom=True, conserve_sz=True, conserve_su2=False):
	N = int(np.prod(Lvec))
	if conserve_su2:
	...
	# Organize (I,J) = (iσ,jσ') into up+down spin channels ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# Decompose i = (i_up, i_dn), j = (j_up, j_dn) and find all combinations of (i_up, j_up) (where i_dn, j_dn determined by total number conservation)
	# Find all spin-splits (nc_up, na_up)=(# up creators, # up annihilators) where nc_up, na_up ∈ {0,1,...,p}; down-channel gets the remainder (p-nc_up, p-na_up)
	splits = [(nc, nc) for nc in range(p+1)] if conserve_sz else [(nc, na) for nc in range(p+1) for na in range(p+1)]
	# Precompute possible many-body idcs for each (nc,na); e.g. (nc,na)=(3,2) --> all possible pairs of N-bit ints (s_cre, s_ann) where |s_cre| = 3, |s_ann| = 2
	S = {ncna: singleSpinChannelCombos(N, ncna[0], ncna[1], allow_overlap=allow_overlap, returnMomTransfer=conserve_mom, Lvec=Lvec) for ncna in splits}
	...
	for (nc_up, na_up) in splits:  # p ≤ N guarantees at least one split survives
		if conserve_mom:
			...
			pgate = np.empty(cre_up.size * cre_dn.size, dtype=dt_fgate)
			pgate['cre_up'], pgate['cre_dn'] = cartesian_prod(cre_up, cre_dn)   # up-combo × down-combo, cre fields
			pgate['ann_up'], pgate['ann_dn'] = cartesian_prod(ann_up, ann_dn)   # same pairing, ann fields
		pgates.append(pgate)
	return np.concatenate(pgates) if pgates else np.empty(0, dtype=dt_fgate)


def sortGatesByIJ(gateLabels, N):
	# Order gates by the 2N-bit integers (s_I, s_J): s_I varies SLOWEST, s_J FASTEST.
	# Returns the reordered labels and the permutation `order` (apply A[order] to match coefficients).
	labels = np.atleast_1d(gateLabels)
	s_I = labels['cre_up'].astype(np.uint64) | (labels['cre_dn'].astype(np.uint64) << N)   # 2N-bit creation string
	s_J = labels['ann_up'].astype(np.uint64) | (labels['ann_dn'].astype(np.uint64) << N)   # 2N-bit annihilation string
	order = np.lexsort((s_J, s_I))     # last key = primary (s_I, slowest); first key = tie-breaker (s_J, fastest)
	return labels[order], order


# Returns a 2N-bit mask for bits that are occupied in Delta that are bigger than an odd number of candidate specators, i.e. bits NOT in s_IJ <-> I∪J
# Inputs:
	# Delta: int for I'∪ J' = (I\B)∪(J\B) where B = I∩J; the thing that is being counted
	# s_IJ: int for I∪J, i.e. which bits to exclude for candidate spectators; note, if I,J have no overlap, then I∪J = I'∪ J'
	# nbits: number of utilized bits in int representation; 2N where N = tot # sites
# Works with python uints or np uints/arrays of np uints (so can be vectorized)
def _odd_spectator_mask(Delta, s_IJ, nbits):
	y = Delta
	sh = 1 # initial shift
	while sh < nbits:  # loop over shifts which are pwrs of 2
		y = y ^ (y >> sh)
		sh <<= 1       # multiply shift by 2
	# At this point, bit b of y is XOR of all bits ≥ b, i.e. y_b = ⨁_{b'≥b} y_b'
	# y>>1 => now y_b = XOR of all bits strictly > b
	# (1<<nbits)-1 => all 1's bitstring
	# XORing it with s_{IJ} => 1 for all indices NOT occupied by s_{IJ} <-> I ∪ J => 1 for all candidate spectators
	is_spectator = (((1 << nbits) - 1) ^ s_IJ) # flip only first 2N bits of s_{IJ} so get 1 for all indices of candidate spectators
	return (y >> 1) &  is_spectator

# Takes in gate label, which defines τ_{IJ}, and outputs matrix-free exp(i a τ_{IJ}) restricted to a reduced Hil space defined by input basis
...
# Inputs
	...
	# sortOrder (optional): argsort(basis)
	# bs (optional): the sorted basis, sorted by integer value
	# spec_mask (optional): 2N-bit integer whose bit rep is a mask for if 1) each bit idx is a candidate spectator given I,J and 2) if I'∪J' have an odd # of indices > each bit
# Returns
	# exp(iaτ_{IJ}): scipy LinearOperator object
def _getGateOperatorSector(g, N, a, basis, sortOrder=None, bs=None, spec_mask=None):
	basis=np.asarray(basis, dtype=np.uint64)
	d=basis.size
	# Int representions of occupations ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# Get 2N-bit integers for cre and ann strings; first N bits for up, last N bits for down
	s_I = int(g['cre_up']) | (int(g['cre_dn']) << N)          # 2N-bit creation string
	s_J = int(g['ann_up']) | (int(g['ann_dn']) << N)          # 2N-bit annihilation string
	s_Ip = s_I & ~s_J; s_Jp = s_J & ~s_I                      # I' = I\J , J' = J\I
	Delta = np.uint64(s_I ^ s_J)                              # I'∪J' = bits where I and J differ
	s_IJ = s_I | s_J                                          # I∪J = bits where either I or J are occupied
	p = bin(s_J).count('1')                                   # order (# annihilators = # creators)
	# Get np int representations for faster vectorized bitwise operations
	sI, sJ, sIp, sJp = np.uint64(s_I),np.uint64(s_J),np.uint64(s_Ip),np.uint64(s_Jp)

	# Compute valid J-src states ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	srcJ_mask=((basis & sJ)==sJ) & ((basis & sIp)==np.uint64(0))  # bool mask for J occupied, I'=I∖J empty; src for C^†_I C_J
	# Note: if I=J, above reduces to basis & s_J
	isrcJ=np.flatnonzero(srcJ_mask).astype(np.intp) # idcs of J-src states
	srcJ=basis[isrcJ] # actual ints for J-src states

	# JW sign from ordering remaining occupied C^†_I C^†_S|0> where S = spectators ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# sign = (-1)^{p(p-1)/2} • (-1)^{#(I'∪J')>r ∀ spectators S}
	sign0 = 1 if (p*(p-1)//2)%2==0 else -1 # bare sign
	# spec_mask flags candidate spectator bits l ∉ I∪J sitting below an ODD # of I'∪J' modes -> occupying such an l contributes a (-1) to the sign
	if spec_mask is None:
		spec_mask=_odd_spectator_mask(Delta, s_IJ, 2*N)
	spec_parity = fastXOR(srcJ & np.uint64(spec_mask)).astype(np.int64) # parity 0=even, 1=odd of #{occupied idcs in srcJ AND which are spectators that contribute (-1) sign}
	sgn=(sign0*(1-2*spec_parity)).astype(np.float64) # full JW sign; Note: if I=J, then spec_parity=0 and sgn = sign0 = (-1)^{p(p-1)/2}
	
	if s_I == s_J: # I = J case ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# τ_{JJ} = 2C^†_J C_J = 2•sign0•P_J (diagonal operator) ==> τ_{JJ}|J∪S> = 2•sign0|J∪S> where sign0 = (-1)^{p(p-1)/2}
		# exp(iaτ_{JJ}) = 1 + (e^{i2a sign0} - 1)P_J ==> exp(iaτ_{JJ})|J∪S> = e^(i2•a•sign0)|J∪S> 
		def _apply(v, phase):
			v = np.asarray(v); shp = v.shape; v = v.ravel()
			w = v.astype(np.complex128, copy=True) # exp(iaτ) acts as identity on non I-src states because τ|non I-src> = 0 and exp(ia•0) = 1
			if isrcJ.size: 
				w[isrcJ] = phase * v[isrcJ]  # exp(iaτ_{II}) scales nontrivially only on I-src states
			return w.reshape(shp)
		phase = np.exp(2j * a * sgn)                            # e^{2ia*sigma0} per source (uniform)
		return sla.LinearOperator((d, d), matvec=lambda v: _apply(v, phase),  rmatvec=lambda v: _apply(v, np.conj(phase)), dtype=np.complex128)

	else:  # I ≠ J case ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		
		# Find corresponding tgt state occupancy ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		srcI_mask=((basis & sI)==sI) & ((basis & sJp)==np.uint64(0))  # bool mask for I occupied, J'=J∖I empty; src for C^†_J C_I
		# C^†_I C_J|J-src> = sgn|I-src>
		# sort basis first so tgt/I-src lookup is fast
		if sortOrder is None: 
			sortOrder=np.argsort(basis, kind='stable') # sorted by integer value
		if bs is None: bs=basis[sortOrder]
		srcI = srcJ^Delta  # can show (R=J∪(R\J) occupied, I empty)^(J and I differ) = (I∪(R\I) occupied, J empty)
		pos=np.minimum(np.searchsorted(bs, srcI), d-1) # if tgt in basis returns its sorted index; if not, returns idx of where it _would_ be in sorted basis, or clamps to value d-1
		
		# Check closure on sector ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# Gate must act within sector: every J-src gets mapped by C^†_I C_J to an I-src state (I + spectators occupied) and vice versa, so τ_{IJ} closed on basis <==> each pair of partners is present
		# suffices to check: 1) for every J-src state present in basis, it's partner is present AND 2) there are no extra I-src states whose partner J-src states aren't present 
		if (srcJ_mask.sum()!=srcI_mask.sum()) or (isrcJ.size and not np.all(bs[pos]==srcI)): # if (#J-src in basis)!=(#I-src in basis) OR (J nonempty but basis does not contain its target)
		...
		isrcI=sortOrder[pos].astype(np.intp) # idx of I-src state in original basis

		# Define action of exp(iaτ_{IJ}) on arbitrary vector v ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# τ_{IJ} = C^†_I C_J + C^†_J C_I
		# exp(iaτ_{IJ}) = (sgn•i(sin a)•σ^x + cos a•1) on subspace spanned by partners |I∪S>, |J∪S>; acts as identity otherwise
		
		# Define action of arbitrary 2x2 unitary matrix U = diag•Id + offdiag•σ^x
		def _apply(v, diag, offdiag):                                         # off=+coef -> U ; off=−coef -> U†
			v=np.asarray(v); shp=v.shape; v=v.ravel()
			w=v.astype(np.complex128, copy=True)                    			# identity action => copy all components of v
			if isrcJ.size:														# if there are candidate J-src states in the reduced basis
				vs=v[isrcJ]; vt=v[isrcI]										# vs = components of v on J-src states; vt = components on J-tgt = I-src states
				w[isrcJ]=diag*vs + offdiag*vt; w[isrcI]=offdiag*vs + diag*vt    # 2x2 matrix on active (src,tgt) blocks
			return w.reshape(shp)
		# exp(iaτ_{IJ}) is special case of the unitary action defined in _apply with a particular choice of diag and off diag
		diag, offdiag =  float(np.cos(a)), (1j*np.sin(a))*sgn # diag, offdiag scales
		return sla.LinearOperator((d,d), matvec=lambda v:_apply(v, diag, offdiag), rmatvec=lambda v:_apply(v,diag,-offdiag), dtype=np.complex128)


# Given a list of gate labels for τ_{IJ} and coeffs A, returns list of matrix-free operators exp(iAτ_{IJ})
# Inputs:
	# gateLabels: 1D np array with dtype fgate; enumerates all
	# A: list or array of real coefficients for each gate
	# N: integer; tot # of sites in lattice
	# basis: basis for reduced Hil space; list or 1D np array of 2N-bit integers for each many-body Fock state with bits [0,N-1] for up and [N,2N-1] for down
# Returns:
	# τlist: list of LinearOperators for each τ_{IJ}, defined to act on reduced Hil space defined by basis (i.e. not necessarily size 2^{2N})
def fgateToExp(gateLabels, A, N, basis):
	labels = np.atleast_1d(gateLabels)
	A = np.broadcast_to(np.atleast_1d(np.asarray(A, dtype=float)), labels.shape) # ensures A is also 1D array of same size as labels
	basis = np.asarray(basis, dtype=np.uint64)
	sortOrder = np.argsort(basis, kind='stable')   
	bs = basis[sortOrder]

	# Vectorized computation of spectator masks over all gate labels
	s_I = labels['cre_up'].astype(np.uint64)|(labels['cre_dn'].astype(np.uint64)<<N)         # 2N-bit cre string, per gate
	s_J = labels['ann_up'].astype(np.uint64)|(labels['ann_dn'].astype(np.uint64)<<N)         # 2N-bit ann string, per gate
	s_IJ = s_I|s_J     # bits occupied by either I or J, per gate
	Delta = s_I^s_J.   # bits occupied by I XOR J; I'∪J'
	spec_masks = _odd_spectator_mask(Delta, s_IJ, 2*N)
	ops = [ _getGateOperatorSector(g, N, a, basis, sortOrder, bs, spec_mask) for g,a,spec_mask in zip(labels, A, spec_masks) ]
