...
function run_summary()
    ...
    println("==========================================================================================================")
    println("                               CONVENTION COMPARISON MATRIX SUMMARY")
    println("==========================================================================================================")
    println("Julia Sign        | Order     | C++ Sign         | Max Mag Diff | Global Phase | +1 Count | -1 Count | Perfect Match?")
    println("----------------------------------------------------------------------------------------------------------")
    ...
    for julia_sign in sign_conventions
        for order_type in orders
            ...
            # Standard x-fastest coordinate mapping
            ...
            for cpp_sign in sign_conventions
                ...
                perfect = (num_neg == 0 && abs_diff < 1e-4) ? "YES (Exact Match)" : "NO (Sign Permuted)"

                @printf("%-17s | %-9s | %-16s | %-12.2e | %-12.1f | %-8d | %-8d | %s\n",
                    string(julia_sign), string(order_name), string(cpp_sign), abs_diff, real(global_phase), num_pos, num_neg, perfect)
            end
        end
    end
    println("==========================================================================================================")
