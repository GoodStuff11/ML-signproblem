...
"""
    mapped_unitary_loss(t_vals_small, state1, state2, subspace_small, subspace_large;
                        order=1, antihermitian=false, spin_symmetry=true, trans_x=true, trans_y=true, conserve_spin=true, omit_H_conj=true,
                        cache=Dict{Symbol, Any}())

Computes the unitary mapping loss `1 - |<state2 | U | state1>|^2` where `U` is generated by 
mapping `t_vals_small` to the large system's symmetry groups. Uses a `cache` to avoid recomputing operators and symmetry groups.
"""
function mapped_unitary_loss(t_vals_small, state1, state2, subspace_small, subspace_large;
    order=1, antihermitian=false, spin_symmetry=true, trans_x=true, trans_y=true, conserve_spin=true, omit_H_conj=true,
    cache=Dict{Symbol,Any}())

    # 1. Map parameters
    t_vals_large, _, t_keys_large, sym_large = map_symmetry_groups(t_vals_small, subspace_small, subspace_large;
        order=order, antihermitian=antihermitian, spin_symmetry=spin_symmetry, trans_x=trans_x, trans_y=trans_y,
        conserve_spin=conserve_spin, omit_H_conj=omit_H_conj, cache=cache)

    # 2. Build or retrieve the large system's sparse operator structure
    if !haskey(cache, :large_operator_structure)
        dim = length(cache[:indexer_large].inv_comb_dict)

        _, parameter_mapping, parity = sym_large

        t_dict_large = cache[:t_dict_large]
        indexer_large = cache[:indexer_large]

        rows, cols, signs, ops_list = build_n_body_structure(t_dict_large, indexer_large)
        param_index_map = build_param_index_map(ops_list, t_keys_large)

        cache[:large_operator_structure] = (rows, cols, signs, param_index_map, parameter_mapping, parity, dim)
    end

    rows, cols, signs, param_index_map, parameter_mapping, parity, dim = cache[:large_operator_structure]

    # 3. Compute loss using fast_loss
    return fast_loss(t_vals_large, rows, cols, signs, param_index_map, parameter_mapping, parity, dim, state1, state2, true, antihermitian)
end
...
function interaction_scan_map_to_state(degen_rm_U::Union{AbstractMatrix,Vector}, instructions::Dict{String,Any}, indexer::CombinationIndexer,
    ...
    if haskey(instructions, "load_file")
        ...
        # starting_loss = dic["metrics"]["loss"][end] # bugged right now
    else
        # starting_loss = Inf
