...
function optimize_unitary(state1::Vector, state2::Vector, indexer::CombinationIndexer;
    ...
    for order ∈ optimization_scheme
        ...
        function f_adjoint_gpu(t_vals, p=nothing)
            ...
            return gpu_adjoint_loss(t_vals, ops_gpu, rows, cols, signs, param_index_map, parameter_mapping, parity, dim, state2_gpu, state1_gpu, p_gpu, !use_symmetry, antihermitian)
        end
        ...
        elseif gradient == :adjoint_gradient && CUDA.has_cuda_gpu()
...
function gpu_fast_loss(t_vals, ops_gpu, rows, cols, signs, param_index_map, parameter_mapping, parity, dim, state1_gpu, state2_gpu, use_symmetry, antihermitian; p_gpu=nothing)
    t = @elapsed begin
        vals = update_values(signs, param_index_map, t_vals, parameter_mapping, parity)
        mat = sparse(rows, cols, vals, dim, dim)
        if !use_symmetry
        ...
        # Ensure CUDA is available via Main or loaded
        mat_gpu = CUDA.CUSPARSE.CuSparseMatrixCSC(mat)
        if p_gpu !== nothing && !(p_gpu isa SciMLBase.NullParameters)
            mat_gpu = mat_gpu + p_gpu
        end
        if antihermitian
            psi_gpu, _ = KrylovKit.exponentiate(mat_gpu, 1.0, state1_gpu; ishermitian=false, tol=1e-12)
            loss = 1 - abs2(dot(state2_gpu, psi_gpu))
        else
            psi_gpu, _ = KrylovKit.exponentiate(mat_gpu, 1.0im, state1_gpu; ishermitian=true, tol=1e-12)
            loss = 1 - abs2(dot(state2_gpu, psi_gpu))
        end
    end
    # println("time=$t loss=$loss")
...
function gpu_adjoint_loss(t_vals, ops_gpu, rows, cols, signs, param_index_map, parameter_mapping, parity, dim, v1_gpu, v2_gpu, p_gpu, do_hermitian, antihermitian=false)
    # 1. Construct A(t) on CPU and move to GPU
    vals = update_values(signs, param_index_map, t_vals, parameter_mapping, parity)
    A = sparse(rows, cols, vals, dim, dim)
    if do_hermitian
        if antihermitian
    ...
    if antihermitian
        psi_gpu, _ = KrylovKit.exponentiate(B_gpu, 1.0, v2_gpu; ishermitian=false, tol=1e-12)
    else
        psi_gpu, _ = KrylovKit.exponentiate(B_gpu, 1.0im, v2_gpu; ishermitian=true, tol=1e-12)
    end
...
function ChainRulesCore.rrule(::typeof(gpu_adjoint_loss), t_vals, ops_gpu, rows, cols, signs, param_index_map, parameter_mapping, parity, dim, v1_gpu, v2_gpu, p_gpu, do_hermitian, antihermitian)
    # --- Reconstruct A ---
    vals = update_values(signs, param_index_map, t_vals, parameter_mapping, parity)
    A = sparse(rows, cols, vals, dim, dim)
    if do_hermitian
    ...
    # First, compute forward state and overlap for the primal return
    if antihermitian
        psi_gpu, _ = KrylovKit.exponentiate(A_gpu, 1.0, v2_gpu; ishermitian=false, tol=1e-12)
    else
        psi_gpu, _ = KrylovKit.exponentiate(A_gpu, 1.0im, v2_gpu; ishermitian=true, tol=1e-12)
    end
    overlap = dot(v1_gpu, psi_gpu)
    y = 1 - abs2(overlap)
