...
## 5. Dataset Targets & Output Modes

### A. Mathematical Target Decompositions
For a given physical context entry $i$ in the dataset, let the vector of raw target variational coefficients be:
$$\mathbf{c}^{(i)} = \left[c_1^{(i)}, c_2^{(i)}, \dots, c_{N_s}^{(i)}\right]^T$$
where $N_s$ is the total number of scattering operator labels.
To train the decoupled shape-scale networks in scaled mode (`use_scale_head == true`), the raw coefficient vector is decomposed into a target log-scale $Y_{\text{log\_scale}}^{(i)}$ and target shape coefficients $\mathbf{Y}_{\text{normalized}}^{(i)} = \left[Y_{\text{normalized}, 1}^{(i)}, \dots, Y_{\text{normalized}, N_s}^{(i)}\right]^T$.
The Root-Mean-Square (RMS) scale of the raw coefficients is first calculated as:
$$\text{RMS}(\mathbf{c}^{(i)}) = \sqrt{\frac{1}{N_s} \sum_{j=1}^{N_s} \left(c_j^{(i)}\right)^2}$$
Using the RMS scale, the target log-scale $Y_{\text{log\_scale}}^{(i)}$ for the scale head is:
$$Y_{\text{log\_scale}}^{(i)} = \begin{cases} \log_{10}\left(\text{RMS}(\mathbf{c}^{(i)})\right) & \text{if } \text{RMS}(\mathbf{c}^{(i)}) > 0 \\ 0 & \text{otherwise} \end{cases}$$
The normalized shape target $Y_{\text{normalized}, j}^{(i)}$ for each scattering leg coefficient is:
$$Y_{\text{normalized}, j}^{(i)} = \begin{cases} \frac{c_j^{(i)}}{\text{RMS}(\mathbf{c}^{(i)})} & \text{if } \text{RMS}(\mathbf{c}^{(i)}) > 0 \\ c_j^{(i)} & \text{otherwise} \end{cases}$$

### B. Scaled Mode (`use_scale_head == true`)
...
### C. Unscaled Mode (`use_scale_head == false`)
